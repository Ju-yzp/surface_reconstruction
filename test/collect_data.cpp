...
class Camera{
    ...
    void processFrame(std::shared_ptr<ob::FrameSet> frames){
        ...
        if(rgb_frame && depth_frame){
           std::shared_ptr<cv::Mat> rgb = std::make_shared<cv::Mat>(cv::Size(1920,1080),CV_8UC4,rgb_frame->data());
           std::shared_ptr<FrameData> fd = std::make_shared<FrameData>(rgb_frame,rgb,depth_frame);
           fd->frame = rgb_frame;
           fd->img = rgb;
    ...
    void start(){
        display_thread_ = std::make_unique<std::thread>(&Camera::displayThread,this);
    }
    ...
    struct FrameData{
        FrameData(std::shared_ptr<ob::ColorFrame>& frame_, std::shared_ptr<cv::Mat>&img_, std::shared_ptr<ob::DepthFrame>& depth_):
